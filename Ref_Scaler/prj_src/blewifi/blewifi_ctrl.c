...
extern unsigned char g_IutIpAddrHex[4]; // added by dong, 2019/03/09
...
void BleWifi_Ctrl_WifiStatusSet(uint8_t status)
{
    ...
    g_ubAppCtrlWifiStatus = status;
	  ...
	  // status= true -> iApConnFlag = AP_CONNECTION_SUCCESS
	  if(status == true) {
			  iApConnFlag = AP_CONNECTION_SUCCESS;
		}
		else {
			  iApConnFlag = AP_CONNECTION_BROKEN;
		}
	  scaler_update_ap_conn(iApConnFlag, g_IutIpAddrHex);	
}
...
void BleWifi_Ctrl_TaskEvtHandler(uint32_t evt_type, void *data, int len)
{
	switch (evt_type)
    {
        ...
        case BLEWIFI_CTRL_MSG_WIFI_CONNECTION_IND:
            ...
            BleWifi_Ctrl_WifiStatusSet(true);  //Harris0312
